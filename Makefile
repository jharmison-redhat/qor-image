...
qor-rpm/qor-$(QOR_VERSION).el9.x86_64.rpm:
	@echo Please put the QOR RPM in $@ >&2
	@exit 1

.build: Containerfile qor-rpm/qor-$(QOR_VERSION).el9.x86_64.rpm overlays/auth/etc/ostree/auth.json $(shell git ls-files | grep '^overlays/') overlays/users/usr/local/ssh/core.keys
...
.PHONY: iso
iso: boot-image/qor-bootc-rhcos$(ISO_SUFFIX).iso

.PHONY: burn
burn: boot-image/qor-bootc-rhcos$(ISO_SUFFIX).iso
